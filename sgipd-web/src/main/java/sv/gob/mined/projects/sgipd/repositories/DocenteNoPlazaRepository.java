...
@ApplicationScoped
public class DocenteNoPlazaRepository extends GenericRepository<DocentesNoplaza>{
    ...
    public DocenteNoPlazaRepository(){
        super(DocentesNoplaza.class);
    }

    @Transactional
    public void merge(DocentesNoplaza dp) {
    ...
    private String getBackupQuery(){
        return "SELECT dp.* INTO Docentes_noplaza_"+
                CalendarUtil.formatSGIPD(new Date())
                +"  FROM Docentes_noplaza dp ";
    }
    ...
    public int deleteAll(){
        return  getEntityManager()
                .createQuery("DELETE FROM DocentesNoplaza dp")
                .executeUpdate();
    }

    @Transactional
    public void mergeList(List<DocentesNoplaza> lista) {
    ...
    @Transactional
    public DocentesNoplaza fromDocenteResumen2DocentesNoplaza(DocenteResumen dr,String currentUser){
        DocentesNoplaza dp = new DocentesNoplaza();
    ...
    @Transactional
    public List<DocentesNoplaza> fromPlaza_2_DocenteNoplazaList(Plaza plaza, String currentUser){
        List<DocenteResumen> nontop10 = docenteResumenRepository.findByPlazaNonTop10(plaza.getIdSecuencial());
        List<DocentesNoplaza> data = new ArrayList<>();
        nontop10.stream().forEach(t->data.add(fromDocenteResumen2DocentesNoplaza(t,currentUser)));
        return data;
    }
}
